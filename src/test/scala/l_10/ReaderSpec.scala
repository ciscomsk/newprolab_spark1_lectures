...
class ReaderSpec extends AnyFlatSpec with should.Matchers with SparkSupport {
  ...
  "Reader" should "read" in {
     ...
     */
    val schemaDDL: String = "id INT,value STRING" // "id INT NOT NULL,value STRING NOT NULL" - Spark 3.5.1

    val df: DataFrame =
      spark
        .readStream
        .schema(StructType.fromDDL(schemaDDL)) // add 1
    ...
    /** 2 партиции (def getPartitions) по 3 элемента в партиции (def compute -> new Iterator[InternalRow]) = 6 элементов в микробатче */
     ...
     * -> 7. java.lang.AssertionError: assertion failed: DataFrame returned by getBatch from org.apache.spark.sql.MyDataSource@57c2272d did not have isStreaming=true
