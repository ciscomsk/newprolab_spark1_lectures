...
object Datasource_4 extends App {
   ...
   * ident - PARTITION KEY, если бы был еще и CLUSTERING KEY - знали бы сортировку внутри партиции.
   *
  ...
  println(ddlQuery) // == CREATE TABLE IF NOT EXISTS airports.codes (ident text PRIMARY KEY,type text,name text,elevation_ft int,continent text,iso_country text,iso_region text,municipality text,gps_code text,iata_code text,local_code text,coordinates text);
  println()

  /** Можно указать несколько - остальные будут найдены через механизм дискавери. */
  spark.conf.set("spark.cassandra.connection.host", "127.0.0.1")

  /**
   * Сколько хостов должно дать одинаковый ответ, для того, чтобы запрос на запись считался успешным.
   * ANY - любой из хостов подтверждает запись (данные не будут потеряны даже в случае отсутствии хостов,
   * на которые по хэшу должны были попасть эти данные, но они не будут доступные до поднятия хотя бы 1 такого хоста).
   * QUORUM - нужен ответ от кворума хостов.
   * ALL - нужен ответ от всех хостов.
   */
  spark.conf.set("spark.cassandra.output.consistency.level", "ANY")

  /**
   * Сколько хостов должно дать одинаковый ответ, для того, чтобы запрос на чтение считался успешным.
   * ONE - первый ответ от любого из узлов, считается правильным.
...
//    .save
  ...
  val cassandraDf: DataFrame = spark
    .read
    .format("org.apache.spark.sql.cassandra")
    .options(tableOpts)
    .load
